...
import { 
  ...
  Clapperboard, Layout as LayoutIcon, Target, Users, 
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isPublicRoute = location.pathname === '/landing' || location.pathname.startsWith('/share');
    ...
    <div className="flex h-screen overflow-hidden bg-dark-bg text-zinc-100 font-sans selection:bg-brand-500/30">
      ...
      <aside className={`${isCollapsed ? 'w-20' : 'w-64'} border-r border-dark-border flex flex-col bg-dark-surface/50 backdrop-blur-xl z-20 shrink-0 transition-all duration-300 relative`}>
        ...
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
          <SidebarItem collapsed={isCollapsed} to="/" icon={LayoutDashboard} label="Dashboard" active={location.pathname === '/' || location.pathname === '/simulator' || location.pathname === '/scheduler' || location.pathname === '/battle'} />
          <SidebarItem collapsed={isCollapsed} to="/extract" icon={Sparkles} label="Brand Extraction" active={location.pathname === '/extract'} />
          ...
          <SidebarItem collapsed={isCollapsed} to="/video" icon={Clapperboard} label="Video Studio" active={location.pathname === '/video'} />
          ...
          <SidebarItem collapsed={isCollapsed} to="/automations" icon={Zap} label="Automations" active={location.pathname === '/automations'} />
          <SidebarItem collapsed={isCollapsed} to="/settings" icon={Settings} label="Settings" active={location.pathname === '/settings'} />
        </nav>

        <div className={`p-4 border-t border-dark-border/50 ${isCollapsed ? 'flex justify-center' : ''}`}>
          {isCollapsed ? (
